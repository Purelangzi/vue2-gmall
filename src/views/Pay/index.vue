...
export default {
  ...
  methods: {
    // 获取订单支付信息
    async getPayInfo(){
        const {data} = await this.$API.reqPayInfo(this.orderId)
        if(data.code==200){
    ...
    async open() {
      // 生成二维码(地址)
      const codeUrl = await QRCode.toDataURL(this.payInfo.codeUrl)
      this.$alert(`<img src="${codeUrl}">`, '微信支付', {
        ...
        beforeClose:(type,instance,done)=>{
          // type区分取消或确定,instance当前组件实例,done关闭弹出框的方法
          if(type == 'cancel'){
